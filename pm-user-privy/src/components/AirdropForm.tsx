...
export default function AirdropForm({ isUnsafeMode, onModeChange }: AirdropFormProps) {
    ...
    useEffect(() => {
        ...
        fetchTokenData()
    }, [tokenAddress, user?.wallet, embeddedWallet?.chain])


    async function handleSubmit() {
        if (!embeddedWallet) return
        const contractType = isUnsafeMode ? "no_check" : "tsender"
        const tSenderAddress = chainsToTSender[embeddedWallet.chainId][contractType]
        ...
        const approvedAmount = await publicClient.readContract({
            ...
            args: [user?.wallet?.address as `0x${string}`, tSenderAddress as `0x${string}`],
        }) as number
